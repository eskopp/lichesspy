...
class DefaultApiClient(object):
    ...
    def call(
        ...
        while True:
            ...
            if post_data:
                ...
                resp = requests.get(
                    url, params, headers=headers, cookies=cookies, stream=stream
                )
    ...
    def on_rate_limit(self, url, retry_count):
        ...
        Raises an exception when :data:`~lichesspy.api.DefaultApiClient.max_retries` is exceeded.
        """
...
def users_by_team(team, **kwargs):
    ...
    >>> users = lichesspy.api.users_by_team('coders')
...
def tournament_standings_page(tournament_id, **kwargs):
    """Wrapper for the `GET /api/tournament/<tournamentId> <https://github.com/ornicar/lila#get-apitournamenttournamentid-fetch-one-tournament>`_ endpoint.
    Use :data:`~lichesspy.api.tournament_standings` to avoid manual pagination.
    """
